...
def send_message_to_gpt4(prompt):
    response = None
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "user", "content": prompt},                # A mensagem do tipo "user" representa a entrada do usuário na conversa.
            {"role": "assistant", "content": "New Document."},  # Nesse caso específico, a mensagem "New Document." indica que o assistente está informando sobre um novo documento, provavelmente para estabelecer um contexto ou iniciar uma nova etapa na conversa.
        ],
